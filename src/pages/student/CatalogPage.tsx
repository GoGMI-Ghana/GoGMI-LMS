...
export default function CatalogPage() {
  ...
  const filteredCourses = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(c => c.title.toLowerCase().includes(q) || c.description.toLowerCase().includes(q) || c.tags.some(t => t.toLowerCase().includes(q)));
    }
    ...
    <div>
      ...
      {filteredCourses.length > 0 ? (
          ...
          {filteredCourses.map(course => {
            ...
            return (
              <div key={course.id} onClick={() => navigate("/catalog/" + course.id)} className="bg-white border border-gray-200 rounded-lg overflow-hidden cursor-pointer hover:border-gray-300 hover:shadow-md transition-all flex flex-col">
                <div className={`h-40 ${course.thumbnailColor} flex items-center justify-center relative`}>
        ...
        <div className="bg-white border border-gray-200 rounded-lg py-16 flex flex-col items-center justify-center">
          ...
          <button onClick={() => { setSearch(""); setSelectedCategory("All"); }} className="text-[13px] font-medium text-brand-teal hover:underline cursor-pointer">Clear all filters</button>
        </div>
      )}
    </div>
  );
