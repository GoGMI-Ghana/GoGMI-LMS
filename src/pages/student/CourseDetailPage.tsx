...
export default function CourseDetailPage({ courseId, onBack }: Props) {
  ...
  if (!course) {
    return (
      <div className="bg-white border border-gray-200 rounded-lg py-16 flex flex-col items-center">
        ...
        <button onClick={onBack} className="text-[13px] text-brand-teal font-medium hover:underline cursor-pointer mt-2">Back to catalog</button>
      </div>
    );
  }
  ...
  return (
    <div>
      <button onClick={onBack} className="flex items-center gap-1.5 text-[13px] text-gray-500 hover:text-gray-700 cursor-pointer mb-5 transition-colors">
            ...
            <div className="flex border-b border-gray-200">
              {(["overview", "syllabus", "facilitators"] as const).map(tab => (
                <button
                  ...
                  className={`px-5 py-3 text-[13.5px] font-medium capitalize transition-colors cursor-pointer ${activeTab === tab ? "text-brand-navy border-b-2 border-brand-navy" : "text-gray-500 hover:text-gray-700"}`}
                >
                  {tab}
                </button>
              ))}
            ...
            <div className="p-6">
              {activeTab === "overview" && (
                <div>
                  ...
                  {course.targetGroup && (
                    <div className="mb-5">
                      <h3 className="text-[13px] font-semibold text-gray-800 mb-2">Who is this for</h3>
                      <p className="text-[13px] text-gray-500 leading-relaxed">{course.targetGroup}</p>
                    </div>
                  )}

                  {course.format && (
                    <div className="mb-5">
                      <h3 className="text-[13px] font-semibold text-gray-800 mb-2">Format & Delivery</h3>
                      <p className="text-[13px] text-gray-500 leading-relaxed">{course.format}</p>
                    </div>
                  )}

                  {course.outcomes.length > 0 && (
                    <div>
                      <h3 className="text-[13px] font-semibold text-gray-800 mb-2">Expected Outcomes</h3>
              ...
              {activeTab === "syllabus" && (
                <div>
                  ...
                  {course.syllabus.length === 0 ? (
                    <p className="text-[13px] text-gray-500">Syllabus details coming soon.</p>
                  ) : (
                        ...
                        <div key={i} className="border border-gray-100 rounded-md overflow-hidden">
                          ...
                          >
                            <div className="flex items-center gap-3 text-left">
                              <span className="text-[12px] font-medium text-gray-400 w-12 shrink-0">M{i + 1}</span>
                              <span className="text-[14px] font-medium text-gray-800">{module.title}</span>
                            </div>
                            <div className="flex items-center gap-3 shrink-0">
                              <span className="text-[12px] text-gray-500">{module.lessons.length} session{module.lessons.length !== 1 ? "s" : ""}</span>
                                ...
                                <div key={j} className="flex items-center justify-between py-2.5 border-b border-gray-50 last:border-0">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      <div className="text-[11px] text-gray-400">{lesson.facilitator}</div>
                                    </div>
                                  </div>
              ...
              {activeTab === "facilitators" && (
                <div>
                  ...
                  <div className="flex flex-col gap-3">
                    {course.facilitators.map((f, i) => (
                      <div key={i} className="flex items-center gap-3 py-2">
                        <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-[12px] font-semibold text-gray-600">
                          {f.split(" ").filter(n => n.length > 1 && !n.includes("(")).map(n => n[0]).join("").slice(0, 2)}
                        </div>
        ...
        <div className="flex flex-col gap-5">
          <div className="bg-white border border-gray-200 rounded-lg p-5 sticky top-20">
            <div className="text-center mb-4">
              <span className={`text-[28px] font-semibold ${course.price === 0 ? "text-green-600" : "text-gray-800"}`}>
                {course.price === 0 ? "Free" : `GHS ${course.price.toLocaleString()}`}
              </span>
            </div>
              ...
              <div>
                ...
                <div className="mt-2">
                  <div className="flex justify-between text-[12.5px] mb-1.5">
                    ...
                    <span className="font-semibold text-gray-700">{enrolled.progress}%</span>
                  </div>
                  <ProgressBar value={enrolled.progress} height="h-[5px]" />
                </div>
              </div>
            ) : (
              <button className="w-full bg-brand-navy text-white rounded-md py-2.5 text-[14px] font-medium cursor-pointer hover:bg-brand-navy-light transition-colors">
            ...
            <div className="mt-5 pt-5 border-t border-gray-100">
              ...
              <div className="flex flex-col gap-2.5">
                {[
                  course.modules > 0 ? `${course.modules} modules, ${totalLessons} sessions` : null,
                  course.duration !== "TBA" ? `${course.duration} duration` : null,
                  "Certificate of completion",
                  "CPD points upon completion",
                  "Recorded sessions for review",
                ].filter(Boolean).map((item, i) => (
                  <div key={i} className="flex items-center gap-2.5">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="text-brand-teal shrink-0"><polyline points="20 6 9 17 4 12" /></svg>
          ...
          {course.tags.length > 0 && (
            <div className="bg-white border border-gray-200 rounded-lg p-5">
              <h3 className="text-[13px] font-semibold text-gray-800 mb-3">Topics</h3>
              <div className="flex flex-wrap gap-2">
                {course.tags.map(tag => (
                  <span key={tag} className="bg-gray-100 text-gray-600 text-[12px] px-2.5 py-1 rounded-md">{tag}</span>
                ))}
              </div>
            </div>
          )}
