...
export default function CourseContentPage() {
  ...
  const { data: completedLessons, refetch: refetchCompletions } = useApi<string[]>("/courses/" + courseId + "/completions");
  ...
  const progress = access?.enrollment?.progress || (totalLessons > 0 ? Math.round((completedCount / totalLessons) * 100) : 0);
            ...
            <div className="mt-4">
              {isLessonDone ? (
                ...
                <button onClick={handleMarkComplete} disabled={marking} className="w-full bg-brand-teal text-white rounded-lg py-3 text-[14px] font-medium cursor-pointer hover:bg-brand-teal/90 transition-colors flex items-center justify-center gap-2">
                  {marking ? "Saving..." : (
                    <><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg>Mark as Complete</>
                  )}
                </button>
              )}
            </div>
                  ...
                  return (
                    <button key={lesson.id} onClick={() => goToLesson(mIdx, lIdx)} className={`w-full text-left px-4 py-2.5 flex items-center gap-3 border-b border-gray-50 cursor-pointer transition-colors ${isActive ? "bg-brand-teal/5 border-l-2 border-l-brand-teal" : "hover:bg-gray-50"}`}>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className={`text-[12px] truncate ${isActive ? "font-semibold text-gray-800" : isDone ? "text-gray-500 line-through" : "text-gray-700"}`}>{lesson.title}</div>
      ...
      {showComplete && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100]">
          <div className="bg-white rounded-xl w-full max-w-md p-8 text-center shadow-2xl">
            ...
            <p className="text-[14px] text-gray-500 mb-6">Congratulations! You've completed all sessions in {course.title}. Your certificate and CPD points will be available shortly.</p>
