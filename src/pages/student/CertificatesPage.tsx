...
export default function CertificatesPage() {
  const points = currentUser.cpdPoints ?? 0;
  const target = currentUser.cpdTarget ?? 40;
  const pct = Math.round((points / target) * 100);
  const radius = 52;
  const stroke = 7;
            ...
            <div className="flex flex-col gap-3">
              {certificates.map(cert => (
                <div key={cert.id} className="bg-white border border-gray-200 rounded-lg p-5 flex items-center gap-4">
                  ...
                  <button className="border border-gray-200 rounded-md px-3.5 py-1.5 text-[13px] font-medium text-gray-600 cursor-pointer hover:bg-gray-50 transition-colors shrink-0">Download</button>
                </div>
              ))}
            </div>
          </div>
                ...
                <tbody>
                  {cpdHistory.map(item => (
                    <tr key={item.id} className="border-b border-gray-50 last:border-0">
                      ...
                      <td className="px-5 py-3.5"><span className={`text-[11px] font-medium px-2 py-0.5 rounded ${item.type === "Course" ? "bg-blue-50 text-brand-navy-muted" : "bg-brand-teal-light text-brand-teal"}`}>{item.type}</span></td>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-5 sticky top-20">
            ...
            <div className="flex flex-col items-center mb-5">
              ...
              <div className="mt-4 text-center">
                <div className="text-[28px] font-semibold text-gray-800">{points}</div>
                <div className="text-[13px] text-gray-500">of {target} points target</div>
              </div>
            </div>
            <div className="flex flex-col gap-3 pt-4 border-t border-gray-100">
              <div className="flex justify-between text-[13px]"><span className="text-gray-500">Current period</span><span className="font-medium text-gray-800">Q2 2026</span></div>
              <div className="flex justify-between text-[13px]"><span className="text-gray-500">Points earned</span><span className="font-medium text-gray-800">{points}</span></div>
              <div className="flex justify-between text-[13px]"><span className="text-gray-500">Remaining</span><span className="font-medium text-brand-teal">{target - points}</span></div>
              <div className="flex justify-between text-[13px]"><span className="text-gray-500">Certificates earned</span><span className="font-medium text-gray-800">{certificates.length}</span></div>
            </div>
            <button className="w-full mt-5 border border-gray-200 rounded-md py-2 text-[13px] font-medium text-gray-600 cursor-pointer hover:bg-gray-50 transition-colors">Export CPD Report</button>
          </div>
        </div>
      </div>
    </div>
