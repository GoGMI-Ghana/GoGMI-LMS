    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-6">
        <div className="w-full max-w-md text-center">
          ...
          <p className="text-[14px] text-gray-500 leading-relaxed mb-6">Thank you for applying as an instructor. Your application is under review by the GoGMI team. You will receive an email once your account has been approved.</p>
        ...
        <div className="relative z-10 flex flex-col justify-between p-10 w-full">
          <div>
            <div className="flex items-center gap-3 mb-16">
              ...
              <div><div className="text-white text-[17px] font-semibold tracking-tight">GoGMI</div><div className="text-white/60 text-[11px] tracking-wide">Learning Platform</div></div>
            </div>
        ...
        <div className="w-full max-w-[440px]">
          ...
          {step === "form" && (
            <div>
              ...
              <p className="text-[14px] text-gray-500 mb-6">{role === "STUDENT" ? "Fill in your details to start learning." : "Fill in your details. Your application will be reviewed by the GoGMI team."}</p>

              {error && <div className="bg-red-50 border border-red-200 rounded-md px-4 py-3 mb-5" role="alert"><p className="text-[13px] text-red-700">{error}</p></div>}
              ...
              <div className="flex flex-col gap-3.5">
                ...
                <div><label className="block text-[13px] font-medium text-gray-700 mb-1">Country</label><select value={country} onChange={e => setCountry(e.target.value)} className="w-full bg-white border border-gray-200 rounded-md px-3.5 py-2.5 text-[14px] text-gray-800 outline-none focus:border-brand-teal transition-colors cursor-pointer"><option value="Ghana">Ghana</option><option value="Nigeria">Nigeria</option><option value="Cameroon">Cameroon</option><option value="Togo">Togo</option><option value="Benin">Benin</option><option value="Senegal">Senegal</option><option value="Gabon">Gabon</option><option value="Angola">Angola</option><option value="Other">Other</option></select></div>
                {role === "INSTRUCTOR" && (
                  <>
                    ...
                    <div><label className="block text-[13px] font-medium text-gray-700 mb-1">Professional bio <span className="text-red-500">*</span></label><textarea value={bio} onChange={e => setBio(e.target.value)} rows={3} placeholder="Briefly describe your professional background..." className="w-full bg-white border border-gray-200 rounded-md px-3.5 py-2.5 text-[14px] text-gray-800 outline-none placeholder:text-gray-400 focus:border-brand-teal focus:ring-1 focus:ring-brand-teal transition-colors resize-none" /></div>
                  </>
                )}
                <div><label className="block text-[13px] font-medium text-gray-700 mb-1">Password <span className="text-red-500">*</span></label><div className="relative"><input type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Minimum 8 characters" className="w-full bg-white border border-gray-200 rounded-md px-3.5 py-2.5 pr-10 text-[14px] text-gray-800 outline-none placeholder:text-gray-400 focus:border-brand-teal focus:ring-1 focus:ring-brand-teal transition-colors" /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 cursor-pointer">{showPassword ? <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94" /><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19" /><line x1="1" y1="1" x2="23" y2="23" /></svg> : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>}</button></div></div>
